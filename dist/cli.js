...
program.version(version).description(description).option("-o, --output <path>", "output directory for bundled files").option("-f, --force", "overwrite output directory").option("-m, --mangle", "mangle variable names").argument("[file]", "input file, defaults to stdin").action(async (input) => {
  const { output, force, mangle } = program.opts();
